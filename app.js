 ...
 **/
var kickstart = require('node-kickstart').withConfig({
    'name': config.host, 
    'port': config.port, 
    'path': __dirname
});
...
srv.error(function(err, req, res, next){
    if (err instanceof NotFound) {
        ...
        mdb.getArticles()
        res.statusCode = 404;
        res.render('errors/404', mdb.jadeData({
            url: req.url,
...
srv.all(/([A-Za-z0-9\-]+\-([0-9]+)\.html)/, function(req, res) {
    ...
    res.render('article', mdb.jadeData({
        article: item, 
        auth: req.session.valid,
        categories:mdb.getCategories()
    }, req));
});
...
srv.all('/feed', function(req, res) {
    return res.render('feed', mdb.jadeData({
        url: mdb.getDefault('url') + req.url, 
        layout: false, 
        list: mdb.getArticles()
    }, req));
});
